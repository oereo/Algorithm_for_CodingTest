...
라이언이 구상한(그리고 아마도 라이언만 즐거울만한) 게임은, 카카오 프렌즈를 두 팀으로 나누고, 각 팀이 같은 곳을 다른 
순서로 방문하도록 해서 먼저 순회를 마친 팀이 승리하는 것이다.
...
void preOrder(Node *root) {
    if (root) {
        preorderAnswer.push_back(root -> num);
        preOrder(root->left);
        preOrder(root->right);
    }
}
...
void postOrder(Node *root) {
    if (root) {
        ...
        postorderAnswer.push_back(root -> num);
    }
}
