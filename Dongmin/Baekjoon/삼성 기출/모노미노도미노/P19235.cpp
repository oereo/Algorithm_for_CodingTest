...
void relocateBlocks(vector<int>& green_removed, vector<int>& blue_removed, int& green_out, int& blue_out) {
    if(green_removed.size() > 0) {
        ...
        int top_green_scanned = *min_element(green_top.begin(), green_top.end());

        for(int r=min_green_removed-1; r > top_green_scanned; r--) {
            for(int c=0; c<4; c++) {
                int& relocated_block = green_board[r][c];
                if(relocated_block != 0) {
                    int located_row = 5;
                    switch(relocated_block) {
                    case 1:
                    case 4:
                    case 5:
                        for(int nr=r+1; nr<=5; nr++) {
                            if(green_board[nr][c] != 0) {
                                located_row = nr-1;
                                break;
                            }
                        }
                        green_board[located_row][c] = relocated_block;
                        relocated_block = 0;
                        break;
                    case 2:
                        int & continuous_relocated_block = green_board[r][c+1];
                        for(int nr=r+1; nr<=5; nr++) {
                            if(green_board[nr][c] != 0 || green_board[nr][c+1]) {
                                located_row = nr-1;
                                break;
                            }
                        }
                        green_board[located_row][c] = relocated_block;
                        green_board[located_row][c+1] = continuous_relocated_block;
                        relocated_block = 0;
                        continuous_relocated_block = 0;
                        break;
                    }
                }
            }
    ...
    if(blue_removed.size() > 0) {
        ...
        int top_blue_scanned = *min_element(blue_top.begin(), blue_top.end());

        for(int r=min_blue_removed-1; r > top_blue_scanned; r--) {
            for(int c=0; c<4; c++) {
                int& relocated_block = blue_board[r][c];
                if(relocated_block != 0) {
                    int located_row = 5;
                    switch(relocated_block) {
                    case 1:
                    ...
                    case 5:
                        for(int nr=r+1; nr<=5; nr++) {
                            if(blue_board[nr][c] != 0) {
                                located_row = nr-1;
                                break;
                            }
                        }
                        
                        blue_board[located_row][c] = relocated_block;
                        relocated_block = 0;
                        break;
                    case 2:
                        int & continuous_relocated_block = blue_board[r][c+1];
                        for(int nr=r+1; nr<=5; nr++) {
                            if(blue_board[nr][c] != 0 || blue_board[nr][c+1]) {
                                located_row = nr-1;
                                break;
                            }
                        }
                        blue_board[located_row][c] = relocated_block;
                        blue_board[located_row][c+1] = continuous_relocated_block;
                        relocated_block = 0;
                        continuous_relocated_block = 0;
                        break;
                    }
